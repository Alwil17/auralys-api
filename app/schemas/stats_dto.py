...
class UserOverallStats(BaseModel):
    ...
    average_sleep: Optional[float] = Field(None, description="Heures de sommeil moyennes")
    ...
    wellness_score: float = Field(..., ge=0, le=100, description="Score de bien-être global (0-100)")
    insights: List[str] = Field(default_factory=list, description="Insights personnalisés")
...
class WeeklyMoodTrend(BaseModel):
    ...
    average_stress: Optional[float] = Field(None, description="Stress moyen de la semaine")
    average_sleep: Optional[float] = Field(None, description="Sommeil moyen de la semaine")
    mood_trend: Literal["improving", "declining", "stable"] = Field(..., description="Tendance de l'humeur")
...
class MoodDistribution(BaseModel):
    ...
    most_common_mood: Optional[int] = Field(None, description="Niveau d'humeur le plus fréquent")
...
class WellnessInsights(BaseModel):
    ...
    insight_type: Literal["sleep", "stress", "regularity", "trend", "general"] = Field(..., description="Type d'insight")
    message: str = Field(..., description="Message d'insight")
    confidence: float = Field(..., ge=0, le=1, description="Niveau de confiance dans l'insight")
    actionable: bool = Field(..., description="L'insight contient-il une action recommandée")
...
class PeriodComparison(BaseModel):
    ...
    current_average_mood: float = Field(..., description="Humeur moyenne période actuelle")
    previous_average_mood: float = Field(..., description="Humeur moyenne période précédente")
    mood_change: float = Field(..., description="Changement d'humeur (positif = amélioration)")
    mood_change_percentage: float = Field(..., description="Pourcentage de changement")
    trend: Literal["much_better", "better", "stable", "worse", "much_worse"] = Field(..., description="Tendance globale")
