...
class NLPService:
    ...
    def _initialize_fallback_models(self):
        ...
        try:
            ...
            logger.error(
                f"Erreur lors de l'initialisation des modèles de fallback: {e}"
            )
            self.emotion_classifier = None
            self.sentiment_classifier = None
    ...
    async def analyze_mood_from_text(self, text: str, language: str = "en") -> Dict:
        ...
        try:
            ...
            return {
                ...
                "emotions": {
                    result["label"]: result["score"] for result in emotion_results[:3]
                },
                "sentiment": (
                    sentiment_results[0]["label"] if sentiment_results else "neutral"
                ),
                "sentiment_confidence": (
                    sentiment_results[0]["score"] if sentiment_results else 0.0
                ),
    ...
    async def _analyze_emotions(self, text: str) -> List[Dict]:
        ...
        try:
            ...
            results = await loop.run_in_executor(None, self.emotion_classifier, text)
    ...
    async def _analyze_sentiment(self, text: str) -> List[Dict]:
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            results = await loop.run_in_executor(None, self.sentiment_classifier, text)
