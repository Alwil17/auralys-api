...
from app.schemas.recommendation_dto import ActivityEffectiveness
...
class StatsService:
    ...
    def get_user_overall_stats(self, user_id: str, days: int = 30) -> UserOverallStats:
        ...
        reco_stats = self.recommendation_repository.get_recommendation_stats(
            user_id, days
        )
    ...
    def get_weekly_mood_trends(
        self, user_id: str, weeks: int = 4
    ) -> List[WeeklyMoodTrend]:
        ...
        for week in range(weeks):
            ...
            if mood_entries:
                ...
                stress_levels = [
                    entry.stress_level for entry in mood_entries if entry.stress_level
                ]
                sleep_hours = [
                    entry.sleep_hours for entry in mood_entries if entry.sleep_hours
                ]
    ...
    def get_mood_distribution(self, user_id: str, days: int = 30) -> MoodDistribution:
        ...
        return MoodDistribution(
            ...
            mood_1_percentage=(
                round((mood_counts[1] / total_entries * 100), 1)
                if total_entries > 0
                else 0
            ),
            mood_2_percentage=(
                round((mood_counts[2] / total_entries * 100), 1)
                if total_entries > 0
                else 0
            ),
            mood_3_percentage=(
                round((mood_counts[3] / total_entries * 100), 1)
                if total_entries > 0
                else 0
            ),
            mood_4_percentage=(
                round((mood_counts[4] / total_entries * 100), 1)
                if total_entries > 0
                else 0
            ),
            mood_5_percentage=(
                round((mood_counts[5] / total_entries * 100), 1)
                if total_entries > 0
                else 0
            ),
            most_common_mood=(
                max(mood_counts, key=mood_counts.get) if total_entries > 0 else None
            ),
        )
    ...
    def get_activity_effectiveness(
        ...
        for activity, stats in activity_stats.items():
            if (
                stats["total"] >= 3
            ):  # Minimum 3 essais pour être statistiquement valable
    ...
    def get_period_comparison(
        ...
        current_entries = self.mood_repository.get_user_mood_entries_by_date_range(
            user_id, current_start.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d")
        )
        ...
        mood_change_percentage = (
            (mood_change / previous_avg) * 100 if previous_avg > 0 else 0
        )
    ...
    def _calculate_wellness_score(
        self, mood_stats: Dict, chat_stats: Dict, reco_stats: Dict
    ) -> float:
    ...
    def _generate_wellness_insights(self, user_id: str, days: int) -> List[str]:
        ...
        if not mood_entries:
            insights.append(
                "Commencez à enregistrer votre humeur quotidiennement pour obtenir des insights personnalisés."
            )
            return insights
        ...
        if len(mood_entries) >= days * 0.8:
            insights.append(
                "Excellente régularité dans le suivi de votre humeur ! Continuez ainsi."
            )
        elif len(mood_entries) >= days * 0.5:
            insights.append(
                "Bon suivi de votre humeur. Essayez d'être encore plus régulier pour de meilleurs insights."
            )
        else:
            insights.append(
                "Essayez d'enregistrer votre humeur plus régulièrement pour identifier des patterns."
            )

        # Analyser les corrélations sommeil-humeur
        entries_with_sleep = [
            e for e in mood_entries if hasattr(e, "sleep_hours") and e.sleep_hours
        ]
        if len(entries_with_sleep) >= 5:
            good_sleep_moods = [
                e.mood for e in entries_with_sleep if e.sleep_hours >= 7
            ]
            ...
            if good_sleep_moods and poor_sleep_moods:
                ...
                if good_avg - poor_avg > 0.5:
                    insights.append(
                        "Votre humeur semble meilleure quand vous dormez bien (7h+). Privilégiez un bon sommeil."
                    )

        # Analyser les tendances de stress
        entries_with_stress = [
            e for e in mood_entries if hasattr(e, "stress_level") and e.stress_level
        ]
        if entries_with_stress:
            avg_stress = sum(e.stress_level for e in entries_with_stress) / len(
                entries_with_stress
            )
            if avg_stress >= 4:
                insights.append(
                    "Votre niveau de stress semble élevé. Pensez à intégrer des activités relaxantes dans votre routine."
                )
            elif avg_stress <= 2:
                insights.append(
                    "Votre gestion du stress semble efficace. Continuez vos bonnes habitudes."
                )
