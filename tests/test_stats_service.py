...
class TestStatsService:
    ...
    def test_get_weekly_mood_trends_4_weeks(self, stats_service, mock_repositories):
        ...
        def mock_get_entries_by_date_range(user_id, start_date, end_date):
            ...
            if start >= datetime.now().date() - timedelta(days=6):
                ...
                return [
                    Mock(mood=2, stress_level=4, sleep_hours=6.0)
                ]
        
        mood_repo.get_user_mood_entries_by_date_range.side_effect = mock_get_entries_by_date_range
    ...
    def test_get_mood_distribution_7_days(self, stats_service, mock_repositories):
        ...
        mock_entries = [
            ...
            Mock(mood=2), Mock(mood=2),  # 2 occurrences
            Mock(mood=3), Mock(mood=3), Mock(mood=3),  # 3 occurrences
    ...
    def test_calculate_wellness_score_various_scenarios(self, stats_service):
        ...
        score = stats_service._calculate_wellness_score(mood_stats, chat_stats, reco_stats)
    ...
    def test_generate_wellness_insights_various_scenarios(self, stats_service, mock_repositories):
        ...
        for i in range(6):  # 6 entrées sur 7 jours = régulier
            entry = Mock()
            entry.sleep_hours = 8.0 if i % 2 == 0 else 6.0  # Alternance bon/mauvais sommeil
            entry.mood = 4 if i % 2 == 0 else 2  # Humeur corrélée au sommeil
            entry.stress_level = 2
            regular_user_entries.append(entry)
        
        mood_repo.get_user_mood_entries_by_date_range.return_value = regular_user_entries
        ...
        assert any(keyword in insights_text for keyword in ["régulier", "données", "suivi"])
        ...
        insights_text = " ".join(insights).lower()
        assert any(keyword in insights_text for keyword in ["commencez", "enregistrer", "données"])
