...
class TestWeeklyAggregation:
    ...
    def test_weekly_aggregation_exact_7_days(self, stats_service_with_real_data):
        ...
        service.mood_repository.get_user_mood_entries_by_date_range.side_effect = (
            mock_get_entries
        )
    ...
    def test_weekly_aggregation_weekend_vs_weekday_patterns(
        self, stats_service_with_real_data
    ):
    ...
    def test_aggregation_with_missing_days(self, stats_service_with_real_data):
        ...
        def mock_get_sparse_entries(user_id, start_date, end_date):
            ...
            return [
                ...
                Mock(
                    mood=3, stress_level=3, sleep_hours=7.0
                ),  # Mercredi (mardi manqué)
                Mock(
                    mood=5, stress_level=1, sleep_hours=8.5
                ),  # Vendredi (jeudi manqué)
                # Week-end manqué
            ]

        service.mood_repository.get_user_mood_entries_by_date_range.side_effect = (
            mock_get_sparse_entries
        )
    ...
    def test_multiple_weeks_aggregation_comparison(self, stats_service_with_real_data):
        ...
        assert (
            trends[0].average_mood
            <= trends[1].average_mood
            <= trends[2].average_mood
            <= trends[3].average_mood
        )

        # Vérifier la progression des données
        assert (
            trends[-1].entries_count >= trends[0].entries_count
