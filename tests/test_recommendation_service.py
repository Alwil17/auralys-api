...
class TestRecommendationService:
    ...
    @pytest.fixture
    def recommendation_service(self, mock_recommendation_repository, mock_mood_repository):
        """Service de recommandations avec mocks"""
        return RecommendationService(mock_recommendation_repository, mock_mood_repository)
    ...
    async def test_generate_recommendations_for_low_mood_level_1(
        ...
        def create_recommendation_side_effect(user_id, reco_data):
            ...
            recommendation.recommendation_type = reco_data.recommendation_type or "mood_based"
        ...
        mock_recommendation_repository.create_recommendation.side_effect = create_recommendation_side_effect
        ...
        # Exécuter
        recommendations = await recommendation_service.generate_recommendations_from_mood(
        ...
        assert mock_recommendation_repository.create_recommendation.call_count == len(recommendations)
    ...
    def test_get_activities_for_low_mood(self, recommendation_service):
        ...
        calming_activities = [a for a in activities_level_1 if a.mood_impact == "calming"]
    ...
    def test_calculate_confidence_score_for_low_mood(self, recommendation_service):
        ...
        score_1 = recommendation_service._calculate_confidence_score(1, calming_activity)
        assert score_1 > 0.7  # Devrait être élevé pour activité appropriée
        
        # Score pour mood level 2 
        score_2 = recommendation_service._calculate_confidence_score(2, calming_activity)
        assert score_2 > 0.7
        ...
        score_energizing = recommendation_service._calculate_confidence_score(1, energizing_activity)
    ...
    def test_select_diverse_activities_for_low_mood(self, recommendation_service):
        ...
        activities = [
            ActivitySuggestion(
                activity="Respiration", description="", estimated_time=5,
                mood_impact="calming", difficulty="easy", category="mental"
            ),
            ActivitySuggestion(
                activity="Douche chaude", description="", estimated_time=15,
                mood_impact="calming", difficulty="easy", category="physical"
            ),
            ActivitySuggestion(
                activity="Appel proche", description="", estimated_time=20,
                mood_impact="positive", difficulty="medium", category="social"
            ),
            ActivitySuggestion(
                activity="Musique douce", description="", estimated_time=10,
                mood_impact="calming", difficulty="easy", category="mental"
            )
        ]
    ...
    async def test_generate_recommendations_with_recent_duplicates(
        ...
        mock_recommendation_repository.get_recent_recommendations.return_value = [recent_reco]
    ...
    async def test_generate_recommendations_with_time_constraints(
        ...
        request = RecommendationGenerateRequest(
            mood_id="mood-123",
            time_available=10  # Seulement 10 minutes
        )
    ...
    @pytest.mark.parametrize("mood_level,expected_mood_impact", [
        (1, "calming"),
        (2, "calming"), 
        (3, "positive"),
        (4, "positive"),
    ...
    def test_mood_level_to_activity_mapping(
        ...
        # Pour les bonnes humeurs, vérifier qu'on a des activités positives/énergisantes  
            ...
            positive_activities = [a for a in activities if a.mood_impact in ["positive", "energizing"]]
