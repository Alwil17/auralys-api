...
from app.schemas.recommendation_dto import RecommendationGenerateRequest, RecommendationUpdate
...
class TestRecommendationIntegration:
    ...
    async def test_full_recommendation_workflow_for_low_mood(
        ...
        assert found_mood is not None, f"Mood entry {mood_entry.id} not found in database"
        ...
        expected_keywords = ["respirer", "musique", "douche", "proche", "méditation", "tisane"]
        ...
        request2 = RecommendationGenerateRequest(
            mood_id=str(user_with_low_mood_data["mood_entries"][1].id),  # Ensure string conversion
            time_available=30
        )
        
        new_recommendations = await recommendation_service.generate_recommendations_from_mood(
            user, request2
        )
    ...
    async def test_recommendation_effectiveness_tracking(
        ...
        # Simuler feedback varié
        helpful_count = 0
        for i, reco in enumerate(all_recommendations):
            # 60% des recommandations marquées comme utiles
            helpful = i % 5 != 0  # 4 sur 5 utiles
            if helpful:
                helpful_count += 1
        ...
        # Devrait avoir des données d'efficacité
        if len(all_recommendations) >= 3:  # Minimum pour stats significatives
            assert len(activity_effectiveness) > 0
            
            for activity_stat in activity_effectiveness:
                assert activity_stat.times_recommended > 0
    ...
    async def test_recommendation_personalization_over_time(
        ...
        request_1 = RecommendationGenerateRequest(
            mood_id=str(mood_entry_1.id),  # Ensure string conversion
            time_available=30
        )
        
        first_recommendations = await recommendation_service.generate_recommendations_from_mood(
            user, request_1
        )
        ...
        request_2 = RecommendationGenerateRequest(
            mood_id=str(mood_entry_2.id),  # Ensure string conversion
            time_available=30
        )
        
        second_recommendations = await recommendation_service.generate_recommendations_from_mood(
            user, request_2
        )
