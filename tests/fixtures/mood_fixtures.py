...
@pytest.fixture
def mood_entries_week(db: Session, test_user: User, sample_mood_data: List[Dict]) -> List[MoodEntry]:
...
def create_test_mood_entries(db: Session, user_id: str, num_entries: int = 30) -> List[MoodEntry]:
    """
...
def get_mood_stats_expected(mood_entries: List[MoodEntry]) -> Dict[str, Any]:
    ...
    stress_levels = [entry.stress_level for entry in mood_entries if entry.stress_level is not None]
    sleep_hours = [entry.sleep_hours for entry in mood_entries if entry.sleep_hours is not None]
